...
extern CurveSetup* SHthorntail_findClosestCurve(f32 x, f32 y, f32 z, s32 curveSubtype);
extern int SHthorntail_animCallback(Object *actor, Object *animObj, AnimObj_Data *animObjData, s8 a3);
extern void SHthorntail_commonControl(Object* self, SHthorntail_Data* objdata, SHthorntail_Setup* setup);
extern void SHthorntail_sleepySetup(Object *self, SHthorntail_Data *objdata, SHthorntail_Setup *setup);
extern void SHthorntail_sleepyInit(Object *self, SHthorntail_Data *objdata, SHthorntail_Setup *setup);
extern void SHthorntail_traderSetup(Object *self, SHthorntail_Data *objdata, SHthorntail_Setup *setup);
extern void SHthorntail_elderSetup(Object *self, SHthorntail_Data *objdata, SHthorntail_Setup *setup);
extern void SHthorntail_updateShadow(Object* self);
extern void SHthorntail_sleepyControl(Object* self, SHthorntail_Data* objdata, SHthorntail_Setup* setup);
extern void SHthorntail_traderControl(Object* self, SHthorntail_Data* objdata, SHthorntail_Setup* setup);
extern void SHthorntail_elderControl(Object* self, SHthorntail_Data* objdata, SHthorntail_Setup* setup);
...
RECOMP_PATCH void SHthorntail_obj_Setup(Object *self, SHthorntail_Setup *setup, s32 reset) {
...
static void SHthorntail_obj_Control_willow_grove(Object* self, SHthorntail_Data* objdata, SHthorntail_Setup* setup) {
    UnkFunc_80024108Struct animInfo;
    TextureAnimator* texAnimA;
    TextureAnimator* texAnimB;

    //Check if gamebit set (wandered off)
    if ((setup->gamebitAway > (NO_GAMEBIT + 1)) && mainGetBits(setup->gamebitAway)) {
        objFreeObject(self);
        return;
    }
    ...
    if (self->curModAnimId != 4) {
        ...
        //Close eyes
        texAnimA = objExprGetTexAnimator(self, 5, 0);
        texAnimB = objExprGetTexAnimator(self, 4, 0);
        if (texAnimA != NULL) {
    ...
    //Advance animation
    objAnimAdvance(self, 0.005f, gUpdateRateF, &animInfo);
}

RECOMP_PATCH void SHthorntail_obj_Control(Object* self) {
    ...
    if (setup->thorntail == THORNTAIL_4_Willow_Grove) {
        SHthorntail_obj_Control_willow_grove(self, objdata, setup);
        return;
    }
...
RECOMP_PATCH void SHthorntail_obj_Free(Object *self, s32 onlySelf) {
...
RECOMP_PATCH u32 SHthorntail_obj_GetDataSize(Object *self, u32 offsetAddr) {
...
RECOMP_HOOK_DLL(SHthorntail_animCallback) void SHthorntail_animCallback_hook(Object *actor, Object *animObj, AnimObj_Data *animObjData, s8 a3) {
    animObjData->unkF4 = thorntail_anim_end_callback;
}

RECOMP_PATCH void SHthorntail_commonControl(Object* self, SHthorntail_Data* objdata, SHthorntail_Setup* setup) {
    s32 ignoreUID;
    s32 allowBranch;
    s32 numBranches;
    s32 numNodeOptions;
    ...
    if (objdata->flags & THORNTAILFLAG_AtTarget) {
        ...
        default:
            if (objdata->flags & THORNTAILFLAG_ProgressionBlocker) {
                ...
                if (mainGetBits(DINOMOD_BIT_921_SH_RiverUnblocked)) {
...
RECOMP_PATCH void SHthorntail_sleepySetup(Object *self, SHthorntail_Data *objdata, SHthorntail_Setup *setup) {
    ...
    if (mainGetBits(DINOMOD_BIT_921_SH_RiverUnblocked)) {
        ...
        objdata->flags &= ~THORNTAILFLAG_RotateTalkSeqs;
    } else if (mainGetBits(BIT_Talked_to_Rocky)) {
        // Sad about the river
...
RECOMP_PATCH void SHthorntail_elderInit(Object *self, SHthorntail_Data *objdata, SHthorntail_Setup *setup) {
...
static _Bool player_has_spellstone(void) {
    return (mainGetBits(BIT_SpellStone_DIM_Activated) && !mainGetBits(BIT_877))
        ...
        || (mainGetBits(BIT_SpellStone_DR)); // Note: Don't revert to non-spellstone state after DR
}

RECOMP_PATCH void SHthorntail_traderAct1Control(Object *self, SHthorntail_Data *objdata, SHthorntail_Setup *setup) {
